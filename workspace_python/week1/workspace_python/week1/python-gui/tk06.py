...
try:
    ...
    messagebox.showinfo("라이브러리 에러 ㅠㅠ", "reportlab 설치되어 있지 않습니다.")
...
def open_file():
    ...
    if file_path:
        with open(file_path, 'r', encoding='utf-8') as f:
            ...
            current_file = file_path  # 열린 파일 경로를 저장
            app.title(f'메모장 - {current_file}')  # 창 제목 변경
...
sub_menu.add_command(label='PDF로저장', command=export_as_pdf)  # PDF 저장 메뉴 연결
sub_menu.add_command(label='TXT로저장')

# '내보내기' 메뉴 항목은 sub_menu를 연계하도록 수정
